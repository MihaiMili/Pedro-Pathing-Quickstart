...
public class RightAuto extends LinearOpMode {
    ...
    public static double sampleCollectAngle = Math.toRadians(270);
    public static double sampleScoreAngle = Math.toRadians(300);
    public static double parkAngle = Math.toRadians(270);
    public static double limit_collect[] = {0.5,0.35,0.35};
    public static double limit_spit[] = {0.4,0.4,0.5};
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!okAuto)
        {
            telemetry.addLine("triunghi 5+1");
            telemetry.addLine("patrat 5+0");
            telemetry.update();
            if (gamepad1.triangle) {newCycle = true;okAuto=true;}
            if (gamepad1.square) {newCycle = false;okAuto=true;}
        }
...
//
        ...
        Pose sampleCollectPose = new Pose(12,44,sampleCollectAngle);
        Pose sampleScorePose = new Pose(10,123,sampleCollectAngle);
        Pose sampleScorePose2 = new Pose(11,128,sampleScoreAngle);
        Pose parkPose = new Pose(9,10,parkAngle);
        ...
        Path sampleCollect = new Path(new BezierLine(new Point(specimen4ScorePose),new Point(sampleCollectPose)));
        sampleCollect.setLinearHeadingInterpolation(specimenCollectAngle,sampleCollectAngle);
        Path sampleScore = new Path(new BezierLine(new Point(sampleCollectPose),new Point(sampleScorePose)));
        sampleScore.setConstantHeadingInterpolation(sampleCollectAngle);
        Path sampleScore2 = new Path(new BezierLine(new Point(sampleScorePose),new Point(sampleScorePose2)));
        sampleScore2.setLinearHeadingInterpolation(sampleCollectAngle,sampleScoreAngle);
        Path park;
       if (newCycle) {
            park = new Path(new BezierLine(new Point(sampleScorePose2), new Point(parkPose)));
           park.setLinearHeadingInterpolation(sampleScoreAngle, parkAngle, 0);
       }
       else
       {
            park = new Path(new BezierLine(new Point(specimen4ScorePose), new Point(specimen1CollectPose)));
           park.setLinearHeadingInterpolation(scoreAngle, 0);
       }
...
//        }
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (status)
            {
                ...
                case COLLECT_CHECK:
                {
                    if (extendopos > 250 && timer_loop.seconds()>0.02)
                    {
                        timer_loop.reset();
                        if (Retrun_Color(r.color_sensor) == "red" || Retrun_Color(r.color_sensor) == "blue")
                    {
                        rightAutoController.CurrentStatus2 = RightAutoController.autoControllerStatus.SHORT_EXTEND;
                ...
                case COLOR_SPIT:
                {
                    ...
                    if (!drive.isBusy() && timer_spit.seconds() > limit_spit[nrcicluri] + 0.1) {
                ...
                case GO_COLLECT_SPECIMEN:
                {
                    ...
                    switch (nrcicluriscorare)
                    {
                        ...
                        {
                            if (newCycle) {
                                drive.followPath(sampleCollect);
                                globals.extendo_auto_i++;
                                extendo_timer.reset();
                ...
                case GO_SCORE_SPECIMEN:
                {
                    ...
                    //if (timer_claw.seconds() > 0.5 && !HasSpecimen(r.claw_sensor)) status = RIghtAuto.STROBOT.FUUNY_HP_1;
                ...
                case GO_COLLECT_SAMPLE:
                {
                    ...
                    {
                        rightAutoController.CurrentStatus2 = RightAutoController.autoControllerStatus.COLLECT_SAMPLE_SHORT;
                ...
                case COLLECT_SAMPLE:
                {
                    if (!drive.isBusy())
                    {
                        rightAutoController.CurrentStatus2 = RightAutoController.autoControllerStatus.COLLECT_SAMPLE_LONG;
                        timer_failsafe.reset();
                        status = STROBOT.COLLECT_SAMPLE_CHECK;
                    }
                    break;
                }
                case COLLECT_SAMPLE_CHECK:
                {
                    //insert verificare senzor
                    if (timer_failsafe.seconds() > 0.2) status = STROBOT.GO_SCORE_SAMPLE;
                    break;
                }
                case GO_SCORE_SAMPLE:
                {
                    drive.followPath(sampleScore);
                    RightAutoController.CurrentStatus = RightAutoController.autoControllerStatus.TRANSFER_BEGIN;
                    status = STROBOT.SCORE_SAMPLE_1;
                    break;
                }
                case SCORE_SAMPLE_1:
                {
                    if (!drive.isBusy() || liftpos > 42000)
                    {
                        drive.followPath(sampleScore2);
                        status = STROBOT.SCORE_SAMPLE_2;
                    }
                    break;
                }
                case SCORE_SAMPLE_2:
                {
                    if (!drive.isBusy() || timer_park.seconds() > 29.5)
                    {
                        RightAutoController.CurrentStatus = RightAutoController.autoControllerStatus.SAMPLE_SCORE;
                        drive.followPath(park);
                        status = STROBOT.PARK;
                    }
                }
                case PARK:
                {
                    ...
                    if (timer_park.seconds() > 29.8 && newCycle)
                    {
                        RightAutoController.CurrentStatus2 = RightAutoController.autoControllerStatus.PARK;
                    }
                    break;
                }
            }
